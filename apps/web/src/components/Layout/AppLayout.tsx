        ...
        <AppShell header={{ height: 56 }} padding={0} style={{ height: '100vh' }}>
            <AppShell.Header>
                <Group h="100%" px="md" justify="space-between">
                    ...
                    <Group gap="xs">
                        <ActionIcon variant="subtle" onClick={toggleLang} title="切换语言 / Toggle Language">
                            <span style={{ fontSize: 13, fontWeight: 600 }}>
                                {i18n.language === 'zh' ? 'EN' : '中'}
                            </span>
                        </ActionIcon>
                        <ActionIcon variant="subtle" onClick={toggleColorScheme} aria-label={t('nav.toggleTheme')}>
            ...
            <AppShell.Main style={{ height: 'calc(100vh - 56px)', overflow: 'hidden' }}>{children}</AppShell.Main>
        </AppShell>
    );
}
